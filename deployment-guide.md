...
```bash
docker build -t synology-office-mcp:0.6.2 -f files/dockers/Dockerfile .
```
...
```bash
docker build -t synology-office-mcp:0.6.2 .
docker save synology-office-mcp:0.6.2 | gzip > synology-office-mcp-0.6.2.tar.gz
scp synology-office-mcp-0.6.2.tar.gz admin@nas:/volume1/docker/
```

Then on the NAS (SSH or Container Manager → Image → Add → From file):

```bash
gunzip -c /volume1/docker/synology-office-mcp-0.6.2.tar.gz | sudo docker load
```
...
sudo docker build -t synology-office-mcp:0.6.2 .
```
